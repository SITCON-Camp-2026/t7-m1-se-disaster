...
export function Phase0JudgementCard({
  ...
  const [draft, setDraft] = useState<Phase0JudgementDraft | null>(
    initial ?? null,
  );
  return (
        ...
        {draft ? (
          <div>
              ...
              >
                {(
                  Object.entries(kindLabels) as Array<
                    [Phase0JudgementDraft["possibleKind"], string]
                  >
                ).map(([k, label]) => (
            ...
            <label>
              ...
              >
                {(
                  Object.entries(confidenceLabels) as Array<
                    [Phase0JudgementDraft["confidence"], string]
                  >
                ).map(([k, label]) => (
                  <option key={k} value={k}>
                    {label}
            ...
            <label>
              ...
              >
                {(
                  Object.entries(nextStepLabels) as Array<
                    [Phase0JudgementDraft["suggestedNextStep"], string]
                  >
                ).map(([k, label]) => (
                  <option key={k} value={k}>
                    {label}
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setDraft({ ...draft, humanReviewNote: e.target.value })
                }
                rows={4}
                style={{ width: "100%" }}
              />
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button
                type="button"
                onClick={() =>
                  onUpdateRecord?.(record.id, { draftJudgement: draft })
                }
              >
                儲存判斷草稿
              </button>
              <button
                ...
                onClick={() => {
                  setDraft(null);
                  onUpdateRecord?.(record.id, { draftJudgement: undefined });
                }}
              >
                刪除判斷草稿
              </button>
              <button
                type="button"
                onClick={() => setDraft(record.draftJudgement ?? null)}
              >
                重設
              </button>
            </div>
          ...
          <div>
            <button
              ...
              onClick={() => {
                ...
                setDraft(newDraft);
                onUpdateRecord?.(record.id, { draftJudgement: newDraft });
              }}
            >
              建立判斷草稿
            </button>
