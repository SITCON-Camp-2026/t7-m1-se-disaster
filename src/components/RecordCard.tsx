...
import {
  ...
  QUALITY_ISSUE_COLORS,
  QUALITY_SEVERITY_COLORS,
  resolveRecordSeverity,
} from "../features/phase-0/quality-issues";
...
export function RecordCard({
  ...
  record: RecordLike;
  onUpdateRecord?: (recordId: string, changes: Partial<RecordLike>) => void;
}) {
            ...
            ? (() => {
                ...
                return (
                  <span
                    style={{
                      padding: "2px 6px",
                      background: QUALITY_SEVERITY_COLORS[sev] ?? "#e9ecef",
                      borderRadius: 4,
                      fontSize: 12,
                    }}
                  >
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
          <button
            type="button"
            onClick={() => {
              onUpdateRecord?.(record.id, { rawText: rawTextValue });
            }}
          >
            儲存原始內容
          </button>
          ...
          {record.qualityIssues.map((q: string) => (
            <span
              ...
              style={{
                marginRight: 6,
                padding: "2px 6px",
                background: QUALITY_ISSUE_COLORS[q] ?? "#e9ecef",
                borderRadius: 4,
                fontSize: 12,
              }}
            >
        ...
        <select
          ...
          onChange={(e) => {
            ...
            onUpdateRecord?.(record.id, {
              qualitySeverity: value || undefined,
            });
          }}
          style={{
            padding: "8px",
